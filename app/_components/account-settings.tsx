...
export function AccountSettings({ account, bookmarkCount }: { account: Account; bookmarkCount: number }) {
  ...
  return <main className="account-page">
    ...
    <div className="account-grid">
      <aside className="account-summary"><span className="account-summary__avatar">{account.name.slice(0, 1).toUpperCase()}</span><strong>{account.name}</strong><span>{account.email}</span><Link href="/bookmarks">{bookmarkCount} saved website{bookmarkCount === 1 ? '' : 's'} →</Link></aside>
